...
void fileSelect(sf::RenderWindow &window, sf::View &view, sf::Font smallFont, sf::Font largeFont, State &state, Transition &transition, std::vector<std::vector<MenuBox>> &menu, sf::Vector2f &menuSelection, bool &enter, bool &escape, SoundManager &soundManager) {
	if (menu.size() == 0) {
		menu.push_back({
			MenuBox(smallFont, smallFontSize, "A", viewWidth / 2 - 90, viewHeight / 2, true, true, false, false, 60, 60, 0)
		});
		menu.push_back({
			MenuBox(smallFont, smallFontSize, "B", viewWidth / 2     , viewHeight / 2, true, true, false, false, 60, 60, 0)
		});
		menu.push_back({
			MenuBox(smallFont, smallFontSize, "c", viewWidth / 2 + 90, viewHeight / 2, true, true, false, false, 60, 60, 0)
		});
	}

	textBox(window, view, largeFont, largeFontSize, "File Select", viewWidth / 2, 15, true, false, false, false, 0, 0, 0, textColor, sf::Color::Transparent, sf::Color::Transparent);
	...
	if (transition.ongoing) {
		...
		if (escape) {
			...
			std::string file = menu.at(menuSelection.x).at(menuSelection.y).string; // todo: save files
			std::cout << file << "\n";
